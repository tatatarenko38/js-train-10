...
function customObjectFromEntries(entries) {
  ...
  const customObject = entries.map((key, value) => {
    if (typeof key !== "string") {
      return [key.toString, (value = key.toString)];
    } else {
      return [key, value.toString];
    }
    return Object.fromEntries(customObject);
  });
}
...
function filterObjectsByKey(arr, key) {
  ...
  const newArr = arr.filter(() => arr.hasOwnProperty(key));
  return newArr;
}
...
function getObjectValuesSum(obj) {
  ...
  if (typeof obj !== "object") {
   ...
   const valuesSum = values.reduce((key, value) => key + value, 0);
   return valuesSum;
}
